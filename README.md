...
- **文件 Agent（受限验收通过）**：从聊天的“工作区”选择单个系统授权目录，再明确同意发送文件内容并开启。模型可列目录、读 UTF-8 文本、提出创建或精确替换；每次写入都需核对修改前后并确认。没有删除、Shell、Root 或目录外访问工具。已在小米专用空目录完成真实 DeepSeek 创建、读取、编辑、原文备份及系统保存界面导出后的回读验证，不代表所有存储提供方和失败场景都通过；具体边界见 `SPEC.md`。
...
3. 描述文件任务。读取的文件名和内容会发给当前配置的 API；文件只是资料，不能自行授予更多权限。写入默认展示增删行、行号和换行类型，可切换查看完整原文与新文本；差异按变更区段计算，并非最小差异，省略时会明确提示。确认仅绑定这一次预览；拒绝即结束该轮文件任务。
